...
local function string_starts(base_string,start)
	return string.sub(base_string,1,string.len(start))==start
end
...
local function check_string_type(value, expected_type)
	...
	for _, typ in ipairs(qualifier) do
		...
		--check class
		if (type(typ) == "string" and string_starts(typ,"class:") and is_subclass(value, string.sub(typ,7))) then
			return true
		end

		local checker = checkers[typ]
	...
	-- argname and function name are formatted by the caller
			'%s', '%s', expected_type, type(value) ~="userdata" and value and value.class and ("class:" .. value.class.name) or type(value)
	)
end
